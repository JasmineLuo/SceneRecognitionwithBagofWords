...
stepsize=4;
%windowsize=30;    %%-----------newly add
%%info from vocab bank;
...
%%get SIFT for one single image
D = vl_alldist2(single(SIFT_features),vocab');

%% could it be a problem since single is used twice ???
%%
sizeofSIFT=size(SIFT_features,2);
line_feat=zeros(1,vocab_size);
%%get distance from test to a set of train results
for j=1:1:sizeofSIFT
    temp=D(j,:);
    [line,ind]=sort(temp);
    line_feat(ind(1))=line_feat(ind(1))+1;
    % take the vote of nearst one    
end
%%get vote for every SIFT feature
%raw_feat=line_feat/sum(line_feat);
raw_feat=line_feat/sum(line_feat);
%%normalization
image_feats(i,:)=raw_feat;
%%image feats add one row for each test picture
i %% ------ for debug
end

%% parameter choices
%   vocab.mat stepsize=5 -> best in second stuation
%   vocab5 stepsize=4 ->best in 3rd stage
